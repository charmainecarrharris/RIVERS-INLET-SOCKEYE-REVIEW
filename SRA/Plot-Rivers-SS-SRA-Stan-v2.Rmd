...
Biological reference points are in hundreds of thousands and log-normal bias corrected. See `SSSR_AR1_v2.stan` for model details. 
```{r}
...
ggsave("./figures/har-age-comp-fit.jpg",height = 4.5, width = 6)
...
Simply zoomed in on recent period observations.
```{r}
...
Summary of stock-recruitment based benchmarks. Lower benchmark is Sgen, upper benchmark is 80% Smsy, both in units of 100,000s.
```{r}
# Sgen1 function
...
Posterior distribution of stock-recruitment based benchmarks and resnet abundance based on SS-SRA (black) and sonar (light grey).
...
ggplot(sub_bench_long, aes(x=value, fill=metric)) + 
  ...
  geom_vline(xintercept=c(bench.quant[3,1],bench.quant[3,2],bench.quant[3,3]),
             ...
             alpha=0.7,
             lwd=1) +
  geom_vline(xintercept = median(tail(stan.data$S_obs, 6)), col="grey", lty=1, lwd =1) 
...
# Extras
Plot of reconstructed spawner abundance based on ratio of clear-stream to sonar estimates for years with data.
```{r}
...
Relationship between clear-stream index and DIDSON counts.
```{r}
...
Expanded escapement timeseries with SR benchmarks
```{r}
